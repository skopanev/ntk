...
///
/// Раньше по нему прекращали устаревшую сессию — 404, и клиент начинал новую.
/// Это оказалось дорогим способом: Claude Code показывает такой ответ человеку
/// как «Authentication expired. Reconnect», и каждая выкатка требовала ручного
/// переподключения у всех. Проверку сняли; о новом наборе клиенту сообщает
/// уведомление.
///
/// Отпечаток остался в идентификаторе сессии, потому что по нему видно, под
/// какой набор она выдана, — это читается в логах и ничего не ломает.
...
async fn call_tool(app: &Arc<App>, token: &str, name: &str, args: &Value) -> (bool, String) {
    ...
    match name {
        ...
        "ntk_ls" => {
            ...
            if args.get("body").and_then(|v| v.as_bool()).unwrap_or(false) { q.push("body=true".into()); }
...
pub async fn endpoint(State(app): State<Arc<App>>, headers: HeaderMap, body: String) -> Response {
    ...
    // Сессию под прежний набор инструментов мы НЕ прекращаем, и это откат.
    //
    // Здесь стояла проверка отпечатка набора: не совпал — 404, клиент обязан
    // начать новую сессию и заново спросить список. По спецификации всё верно,
    // на практике вышло иначе. За два дня она сработала тридцать четыре раза,
    // по пачке на каждую выкатку, и Claude Code показывает это человеку как
    // «Authentication expired. Reconnect» — то есть как проблему со входом,
    // требующую ручного действия. Человек получал «переподключись» трижды за
    // два дня, и ни разу это не было про его доступ.
    //
    // Нужды в ней нет: клиенту и так говорят перечитать список — уведомление
    // едет вместе с ответом на первый же вызов в сессии (rpc_ok_with_notice),
    // а тем, кто держит поток по GET, ещё и при его открытии. Набор меняется
    // только выкаткой, выкатка чистит память процесса, и первый вызов каждого
    // клиента после неё получает уведомление. Разрывать соединение поверх
    // этого — значит платить ручным действием человека за то, что уже сделано.
