...
/// Сколько похожих показываем.
///
/// Было пять — под довод «больше никто не прочитает». Довод верен для человека
/// и неверен для того, кто это читает на самом деле: список разбирает агент, и
/// шестая строка стоит ему не внимания, а одной строки в ответе.
/// А отрезать её стало чем: порог показа опущен до 0.60, попаданий выше него
/// теперь заметно больше пяти, и пятёрка резала бы ровно то, ради чего порог и
/// опускали.
...
pub fn reconcile_ticks(state: &Arc<crate::App>) {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            // Раз в час, а не раз в шесть. Проход дешёвый: читаются метаданные
            // точек и список тикетов, сравнение в памяти; дорого становится
            // только когда расхождение ДЕЙСТВИТЕЛЬНО нашлось, и это польза.
            //
            // Шести часов хватало, пока расхождение появлялось только от сбоя
            // индексатора. Но тикет может ожить в обход записи — так было
            // 21.09, когда пятьдесят удалённых вернули правкой deleted_at, —
            // и тогда долг не ставит никто. Час вместо шести сокращает окно,
            // в котором живой тикет не находится поиском и никто не понимает
            // почему.
