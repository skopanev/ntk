...
pub async fn create(
    ...
    // Обход проверки записывается ИМЕНЕМ. Правило, нарушение которого никто не
    // видит, — это пожелание: за один вечер флаг стал у одного исполнителя
    // рутиной, причём он же в тот день доказывал, что порог надо двигать. Одно
    // исключает другое, и заметить это можно было только по записи.
    if p.skip_search {
        tracing::info!(
            actor = %actor.user_id,
            workspace = %ws,
            "заведение без поиска похожих: обход проверки"
        );
    }
    ...
    if let Some(v) = searching {
        ...
        if stopping {
            ...
            // Поиск похожих ОГРАНИЧЕН ПО ВРЕМЕНИ, и это не украшение.
            //
            // Клиент ждёт ответа 20 секунд. Сервис ждал провайдера до 30, а
            // ограничитель скорости мог усыпить запрос ещё до 60, пока в
            // минутном окне не освободится место. Предела на сам поиск не было
            // вовсе, то есть заведение тикета могло занять полторы минуты —
            // втрое дольше, чем клиент готов ждать.
            //
            // Наружу это выглядело как «the service is unavailable»: сообщение
            // КЛИЕНТА о том, что ответа нет. Люди сообщали, что заведение не
            // работает, тогда как чтения проходили мгновенно, — и были правы.
            //
            // Восемь секунд с запасом укладываются в терпение клиента. Не
            // успели — заводим без поиска, по уже существующей ветке ниже:
            ...
            .await
            .unwrap_or_else(|_| {
                Err(anyhow::anyhow!("поиск похожих не уложился в {SEARCH_BUDGET:?}"))
            });
            match search
            {
...
/// Предел входа embedding: РОВНО столько, сколько даёт законный тикет.
///
/// Здесь стояло 2000 — то же число, что предел тела. Ошибка была в том, что
/// это ДРУГАЯ ручка: запись ограничена по полям (тело 2000, заголовок 256), а
/// вход — по склейке title + '\n' + body, и она законно доходит до 2257.
/// Тикет с длинным заголовком терял хвост тела, оставаясь целиком в пределах.
/// Замер на 4164 боевых тикетах: резалось 431, в среднем на 64 символа,
/// максимум на 213; при 2257 не режется ни один — самый длинный даёт 2213.
///
/// Свой комментарий это и описывал («законный тикет даёт до 2257»), и тут же
/// стояло 2000: увидеть расхождение мало, надо его закрыть.
///
/// Выводится из пределов записи, а не пишется числом: поднимут предел тела —
/// поднимется и этот, без второго места, о котором надо помнить. Единица —
/// перевод строки между заголовком и телом.
///
/// Обрезка остаётся: 2257 — это гарантия для ЗАКОННОГО тикета, а в базе лежат
/// и старые, заведённые до пределов, на сотни тысяч символов.
pub(crate) const EMBED_INPUT_MAX: usize =
    ntk_core::tools::TITLE_MAX + 1 + ntk_core::tools::BODY_MAX;
...
mod embed_input_tests {
    ...
    /// Предел входа обязан покрывать ЛЮБОЙ законный тикет целиком.
    ///
    /// Иначе тикет, полностью укладывающийся в пределы записи, всё равно
    /// теряет хвост при индексации — и заметить это можно только по
    /// ухудшившемуся поиску, то есть никогда.
    #[test]
    fn the_clip_covers_every_legal_ticket() {
        let longest = ntk_core::tools::TITLE_MAX + 1 + ntk_core::tools::BODY_MAX;
        assert_eq!(EMBED_INPUT_MAX, longest, "законный тикет обязан помещаться целиком");
        let title = "t".repeat(ntk_core::tools::TITLE_MAX);
        let body = "b".repeat(ntk_core::tools::BODY_MAX);
        assert_eq!(embed_input(&title, &body).chars().count(), longest);
    }
