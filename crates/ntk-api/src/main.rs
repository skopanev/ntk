...
pub struct App {
    ...
    /// Появилась работа. Отдельно от `draining`, чтобы пробуждение, попавшее в
    /// конец слива, не потерялось: см. `vector::wake`.
    pub vector_pending: std::sync::atomic::AtomicBool,
}
...
async fn main() -> anyhow::Result<()> {
    ...
    // Строка подключения нужна дальше слушателю уведомлений, а cfg уходит в App.
    ...
    let app = Arc::new(App {
        ...
        vector_pending: std::sync::atomic::AtomicBool::new(false),
    });
    ...
    vector::wake(&app);
    // Включение векторизации — UPDATE в базе, а не запрос сюда. Слушаем, чтобы
    // «включили» и «пошло» были одним событием, а не двумя с паузой.
    vector::listen(&app, db_url);
...
pub(crate) async fn tickets(
    ...
    //
    // `-P bakend` в воркспейсе, где проект называется `backend`, отдавал ноль — ту
